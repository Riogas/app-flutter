...
object DeviceEventReporter {
    ...
    fun report(context: Context, tipo: String, motivo: String = "", extra: Map<String, String> = emptyMap()) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            reportInternal(appContext, tipo, motivo, extra)
        }
    }

    /**
     * Igual que report(), pero SIN lanzar una corrutina propia: corre en el dispatcher
     * del caller (vía withContext(Dispatchers.IO)) y se espera a que termine. Pensado
     * para CoroutineWorkers, donde el POST debe completarse antes de Result.success()
     * para que WorkManager mantenga vivo el proceso.
     */
    suspend fun reportAndWait(context: Context, tipo: String, motivo: String = "", extra: Map<String, String> = emptyMap()) {
        val appContext = context.applicationContext
    ...
    private suspend fun reportInternal(appContext: Context, tipo: String, motivo: String, extra: Map<String, String>) {
        try {
